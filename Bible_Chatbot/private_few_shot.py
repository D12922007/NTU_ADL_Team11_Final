...
def perplexity(
    model, tokenizer, data, samples, max_length=2048,
):
    data_size = len(data)
    instructions = [get_prompt_few_shot(data[x]["instruction"], data=samples, x=x, num=args.num_of_in_context) for x in range(data_size)]
    ...
    for i in tqdm(range(data_size)):
        ...
        with torch.no_grad():
            ...
            print("result ",result)
            print("result_start ",result[0].rfind('ASSISTANT'))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_data_path",
        type=str,
        default="'./output_result_private_few.json'",
        required=False,
    ...
    model.eval()
    ppl, data = perplexity(model, tokenizer, data, samples)
