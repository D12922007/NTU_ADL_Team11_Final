...
def perplexity(
    model, tokenizer, data, examples, max_length=2048,
):
    data_size = len(data)
    instructions = [get_prompt_few_shot(data[x]["instruction"], examples, x, args.num_of_in_context) for x in range(data_size)]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_data_path",
        type=str,
        default="'./output_bonus_result_private_few.json'",
        required=False,
    ...
    with open(args.test_data_path, "r") as f:
        data = json.load(f)
    with open(args.example_data_path, "r") as f:
        example_data = json.load(f)
    #print("example_data ",example_data)
    model.eval()
    data = perplexity(model=model, tokenizer=tokenizer, data=data, examples=example_data)

    #with open('output_result_few_1.json', 'w', encoding='utf8') as f:
