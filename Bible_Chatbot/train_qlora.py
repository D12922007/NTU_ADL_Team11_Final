...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    #10000
    output_dir: str = field(default='./output_all', metadata={"help": 'The output dir for logs and checkpoints'})
    optim: str = field(default='paged_adamw_32bit', metadata={"help": 'The optimizer to be used'})
    per_device_train_batch_size: int = field(default=1, metadata={"help": 'The training batch size per GPU. Increase for better speed.'})
    gradient_accumulation_steps: int = field(default=16, metadata={"help": 'How many gradients to accumulate before to perform an optimizer step'})
    max_steps: int = field(default=625, metadata={"help": 'How many optimizer update steps to take'})
    weight_decay: float = field(default=2e-4, metadata={"help": 'The L2 weight decay rate of AdamW'}) # use lora dropout instead for regularization if needed 2e-4
    learning_rate: float = field(default=0.998e-4, metadata={"help": 'The learnign rate'})#1e-4
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def load_data(dataset_name):
        ...
        elif dataset_name == './data/train.json':
...
def train():
    ...
    if args.do_train:
        ...
        with open("./data/public_test.json", "r") as f:
            data = json.load(f)
        ppl = perplexity(trainer.model, tokenizer, data)
