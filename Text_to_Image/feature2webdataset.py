...
if __name__ == "__main__":
    ...
    img_emb_path = "web_img/" #/path/to/img_emb
    text_raw_path = "assets/contexts/run_vis" #/path/to/text_raw
    ...
    def sampler(fp, url):
        ...
        print("np.load(image_path, allow_pickle=True).T[:256]",np.load(image_path, allow_pickle=True).T[:256].shape)
        print("np.load(image_path, allow_pickle=True).T[:256].reshape(-1, 256)",np.load(image_path, allow_pickle=True).T[:256].shape)
        images = np.load(image_path, allow_pickle=True).T[:256].reshape(-1, 256)
    ...
    make_wds_shards(
        pattern=f"output_path/%08d.tar", #f"{output_path}/%08d.tar",
